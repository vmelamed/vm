...
//
// Shows how to use EventSources to send messages to the Windows EventLog
//
// * EventLogEventSource
...
//       manifest on the machine. If running un-elevated it will prompt the user
...
namespace EventSourceSamples
{
    ...
    public sealed class EventLogEventSource : EventSource
    {
        ...
        [Event(1, Keywords = Keywords.Requests, Channel = EventChannel.Admin, Task = Tasks.Request, Opcode = EventOpcode.Start,
            Message = "Start processing request\n\t*** {0} ***\nfor URL\n\t=== {1} ===")]
        public void RequestStart(int RequestID, string Url) { WriteEvent(1, RequestID, Url); }

        [Event(2, Keywords = Keywords.Requests, Channel = EventChannel.Analytic, Task = Tasks.Request, Opcode = EventOpcode.Info, Level = EventLevel.Verbose,
            Message = "Entering Phase {1} for request {0}")]
        public void RequestPhase(int RequestID, string PhaseName) { WriteEvent(2, RequestID, PhaseName); }

        [Event(3, Keywords = Keywords.Requests, Channel = EventChannel.Admin, Task = Tasks.Request, Opcode = EventOpcode.Stop,
            Message = "Stop processing request\n\t*** {0} ***")]
        public void RequestStop(int RequestID) { WriteEvent(3, RequestID); }

        [Event(4, Keywords = Keywords.Debug, Channel = EventChannel.Debug,
            Message = "DebugMessage: {0}")]
    ...
    static class RegisterEventSourceWithOperatingSystem
    {
        ...
        /// <summary>
        /// Simulate an installation to 'destinationFolder' for the named eventSource.  If you don't
        /// specify eventSourceName all eventSources information next to the EXE is registered.
        /// </summary>
        public static void SimulateInstall(string destinationFolder, string eventSourceName = "", bool prompt = true)
        {
            ...
            {
                var destinationPath =  Path.Combine(destinationFolder, Path.GetFileName(filename));
                Out.WriteLine("xcopy \"{0}\" \"{1}\"", filename, destinationPath);
                File.Copy(filename, destinationPath, true);
            }

            Out.WriteLine("Registering the manifest with the OS (Need to be elevated)");
            foreach (var filename in Directory.GetFiles(destinationFolder, "*.etwManifest.man"))
            {
                var commandArgs = string.Format("im {0} /rf:\"{1}\" /mf:\"{1}\"",
                    filename,
                    Path.Combine(destinationFolder, Path.GetFileNameWithoutExtension(filename) + ".dll"));
        ...
        /// </summary>
        public static void SimulateUninstall(string destinationFolder, bool prompt = true)
        {
            Out.WriteLine("Uninstalling the EventSource demos from {0}", destinationFolder);
            ...
            Out.WriteLine("Removing {0}", destinationFolder);
