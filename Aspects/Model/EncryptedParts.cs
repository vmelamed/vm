...
{
    /// <summary>
    /// Class EncryptedParts encrypts and decrypts all properties and fields of any visibility of an object
    /// into the  and from the properties and fields marked by the <see cref="DecryptedAttribute.EncryptedIn"/>.
    /// </summary>
    public class EncryptedParts
    {
        ...
        /// </summary>
        const BindingFlags EncryptedPartsFlags = BindingFlags.DeclaredOnly | BindingFlags.FlattenHierarchy | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance;
#if DEBUG
        const Formatting _formatting = Formatting.Indented;
#else
        const Formatting _formatting = Formatting.None; 
#endif
        ...
        void Verify()
        {
            ...
            if (!mapDecryptedParts.All(g => ((MemberInfo)_type.GetField(g.Key)?.FieldType ??
                                             (MemberInfo)_type.GetProperty(g.Key)?.PropertyType) == typeof(byte[])))
        ...
        /// <summary>
        /// Encrypts the properties and fields of the <see cref="_instance"/>.
        /// </summary>
        public void Encrypt()
        {
            ...
            foreach (var g in decryptedParts)
            {
                ...
                {
                    var pi = mi as PropertyInfo;
                    var fi = pi != null ? mi as FieldInfo : null;
                ...
                var json = jo.ToString(_formatting);

                // if the new JSON is the same as the cached JSON - no need for encryption.
                if (json == _decryptedJsonStrings[g.Key])
                    continue;

                var piEncrypted = _type.GetProperty(g.Key);
                var fiEncrypted = piEncrypted != null ? null : _type.GetField(g.Key);
        ...
        public void Decrypt()
        {
            ...
            foreach (var g in decryptedParts)
            {
                ...
                {
                    var pi = _type.GetProperty(p.Name);
                    var fi = pi != null ? null : _type.GetField(p.Name);
        ...
        IEnumerable<IGrouping<string, MemberInfo>> ListDecryptedParts()
        {
            ...
            using (_lockMapDecryptedParts.UpgradableReaderLock())
            {
                ...
                {
                    mapDecryptedParts = _type.GetProperties(EncryptedPartsFlags)
                                            .AsEnumerable<MemberInfo>()
                                            .Union(
                                                _type.GetFields(EncryptedPartsFlags)
                                                     .AsEnumerable<MemberInfo>())
                                            .Where(mi => mi.IsDefined(typeof(DecryptedAttribute), true))
