...
namespace vm.Aspects.Threading
{
    ...
    public class RetryTasks<T>
    {
        ...
        readonly static Func<T, int, Task<bool>> _defaultFailure = (_,__) => Task.FromResult(true);
        ...
        /// </summary>
        /// <param name="operationAsync">The asynchronous operation to be tried one or more times.</param>
        /// <param name="maxRetries">The maximum number to retry.</param>
        /// <param name="minDelay">The minimum delay before retry in milliseconds.</param>
        /// <param name="maxDelay">The maximum delay before retry in milliseconds.</param>
        /// <param name="isFailureAsync">Caller supplied asynchronous lambda which determines if the most recent operation failed.
        /// If <see langword="null"/> the default returns <see langword="true"/> - the operation is considered failed the first time.
        /// Note that <paramref name="isFailureAsync"/> is always called before <paramref name="isSuccessAsync"/>.
        /// Hint: based on the result from the last operation, the lambda may throw exception.</param>
        /// <param name="isSuccessAsync">Caller supplied asynchronous lambda which determines if the most recent operation succeeded.
        /// If <see langword="null"/> the default returns <see langword="true"/> - the operation is considered successful the first time.
        /// Note that <paramref name="isFailureAsync"/> is always called before <paramref name="isSuccessAsync"/>.</param>
        /// <param name="epilogueAsync">Caller supplied asynchronous lambda to be run after the operation was attempted unsuccessfully <paramref name="maxRetries"/> times.
        /// Hint: based on the result from the last operation, the lambda may throw exception.</param>
        public RetryTasks(
